...
#
# The 6.00 Word Game
# Time problem 1 and 2 takes 1.25 hr
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while sum(hand.values()) > 0:
        ...
        if interval_time < .1: interval_time = .1
        if userWord == '.':
             break
        else:
            ...
            if not isValid:
                print 'It took %.2f seconds to provide an invalid answer. ' % interval_time
                print 'You have %.2f seconds remaining. ' % left_time
                print 'Invalid word, please try again.'
            else:
                if left_time < 0:
                    print 'Total time exceeds %.2f seconds. You scored %.2f points. ' % (abs(left_time), total)
                    normal_exit = False
                    break
                points = get_word_score(userWord, initial_handlen)/interval_time
                total += points
                print 'It took %.2f seconds to provide an answer. ' % interval_time
                print 'You have %.2f seconds remaining. ' % left_time
                print '%s earned %.2f points. Total: %.2f points' % (userWord, points, total)
                hand = update_hand(hand, userWord)
    if normal_exit: print 'Total score: %2f points.' % total
