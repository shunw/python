...
def get_word_score(word, n):
    ...
    total_score = 0
    for alph in word:
        total_score += SCRABBLE_LETTER_VALUES[alph]
    total_score = total_score * qty
    if qty == n:
        total_score += 50
    return total_score    
#
...
def is_valid_word(word, hand, word_list):
    ...
    for i in word: 
        hand_new[i] = hand_new.get(i, 0) - 1
        if hand_new[i] < 0: return False
    return True
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while sum(hand.values()) > 0: 
        ...
        if not(is_valid_word(word_input, hand, word_list)):
            print 'the word: %s is not valid! ' % word_input
            continue
        temp_s = get_word_score(word_input, HAND_SIZE)
        total_s += temp_s
        print '"%s" earned %d points. Total: %d points' % (word_input, temp_s, total_s) 
